functions {
  real vlfunc(real a1, real b1, real t1, real t_onset, real tmax1, real log_vlmax1){
    return(log10(10^log_vlmax1*(a1 + b1)/(b1 * exp(-a1*(t1+t_onset-tmax1)) + a1 * exp(b1*(t1+t_onset-tmax1)))));
  } 
}
...
//  real tmax[K];
//  for(i in 1:K)
//    tmax[K] = tmax_bar + tmax_raw * sigma_tmax;
//}

model{
  for(i in 1:N){
    vl[i] ~ normal(vlfunc(a[individual[i]], b[individual[i]], t[i], t_onset[individual[i]], tmax[individual[i]], log_vlmax[individual[i]]), sigma);
  }
  // priors
  ...
  log_vlmax_bar ~ normal(8,1);
  log_vlmax_sigma ~ normal(0,1);
}